        ...
        <div class="card">
          <div class="filter">
            ...
            <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
              ...
              <li><a class="dropdown-item changeRecentActivityFilter" data-timeDiff="1" href="#">Today</a></li>
              <li><a class="dropdown-item changeRecentActivityFilter" data-timeDiff="3" href="#">Last 3 days</a></li>
              <li><a class="dropdown-item changeRecentActivityFilter" data-timeDiff="7" href="#">Last 7 days</a></li>
            </ul>
          </div>

          <div class="card-body">
            <h5 class="card-title">Recent Activity |<span id="timeDiffSelect"> Today</span></h5>

            <div class="activity">
              @if(isset($activityData['body']['data']) && count($activityData['body']['data']) > 0)
                @include('dashboard.recentActivity', ['logs' => $activityData['body']['data']])
              @endif
            </div>
...
@section('scripts')
  ...
  <script>
    $(document).ready(function () {
      $('.changeRecentActivityFilter').click(function(e) {
        e.preventDefault();
        var el = $(this);
        var timeDiff = el.attr('data-timeDiff');
        console.log(timeDiff);
        $.ajax({
          type: 'GET', 
          url: "{{route('recent.activity')}}?timeDiff="+timeDiff,
          async: false,
          success: function(response) {
            $('.activity').html(response.html);
            $('#timeDiffSelect').html('Last '+timeDiff+' days');
          }
        })
      });
    })
