  ...
  return {
    ...
    getSalesCardInfo: async function (req, res) {
      try {
        ...
        var salesCardVal = await functionTrait.getSalesCardVal(user, storeData, redis, filterArr);
    ...
    index: async function (req, res) {
      try {
        ...
        if(cacheHasData) {
          ...
          returnVal = JSON.parse(await redis.get(cacheKey));
        } else {
          var dashboardData = {
            "summary": await functionTrait.getDashboardSummary(authUser, storeData, redis),
            "recentActivity": await functionTrait.getRecentActivity(authUser, storeData, redis),
            "reports": await functionTrait.getReportData(authUser, storeData, redis),
            "budgetReport": await functionTrait.getBudgetReport(authUser, storeData, redis),
            "recentSales": await functionTrait.getRecentSales(authUser, storeData, redis),
            "websiteTraffic": await functionTrait.getWebsiteTraffic(authUser, storeData, redis),
            "topSellingProducts": await functionTrait.getTopSellingProducts(authUser, storeData, redis)
          };
